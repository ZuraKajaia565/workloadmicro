...
public class MicroservicesIntegrationSteps {
    ...
    public void aNewTrainingSessionIsCreated() {
        ...
        String workloadServiceUrl = "http://localhost:" + workloadService.getMappedPort(8080);
        response = restTemplate.postForEntity(workloadServiceUrl + "/api/training-sessions",
    ...
    public void theNotificationServiceShouldReceiveTheEvent() {
        String notificationServiceUrl = "http://localhost:" + notificationService.getMappedPort(8081);
        response = restTemplate.getForEntity(notificationServiceUrl + "/api/notifications/latest",
                Object.class);
        assertTrue(response.getStatusCode().is2xxSuccessful());
    }
    ...
    public void multipleInstancesOfAServiceAreRunning() {
        ...
        GenericContainer<?> additionalInstance = new GenericContainer<>("workload-service:latest")
                .withNetwork(network)
    ...
    public void aClientMakesARequest() {
        ...
        String serviceUrl = loadBalancerClient.choose("workload-service").getUri().toString();
        response = restTemplate.getForEntity(serviceUrl + "/api/training-sessions", Object.class);
    }
    ...
    public void aServiceIsExperiencingHighLatency() {
        ...
        wireMockServer.stubFor(get(urlPathMatching("/api/training-sessions"))
    ...
    public void multipleRequestsAreMade() {
        ...
        for (int i = 0; i < 5; i++) {
            try {
                response = restTemplate.getForEntity("http://localhost:8089/api/training-sessions",
                        Object.class);
            } catch (Exception e) {
                exception = e;
            }
    ...
    public void allServicesAreRunning() {
        allMicroservicesAreRunning();
    }

    @When("the health check endpoint is called")
    public void theHealthCheckEndpointIsCalled() {
        String workloadServiceUrl = "http://localhost:" + workloadService.getMappedPort(8080);
        response = restTemplate.getForEntity(workloadServiceUrl + "/actuator/health", Object.class);
    }
